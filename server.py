...
async def get_product_categories():
    """Get all unique categories from products (legacy endpoint)"""
    # First try to get from categories collection
    categories = await db.categories.find().sort("name", 1).to_list(100)
    if categories:
        return [c["name"] for c in categories]
    # Fallback to distinct categories from products
...
async def startup_event():
    ...
    default_categories = ['Fruits', 'Vegetables', 'Dairy', 'Bakery', 'Beverages', 'Snacks', 'Grains', 'Spices', 'Other']
